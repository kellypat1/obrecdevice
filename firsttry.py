    ...
    for i in np.arange(0, 70):
        if Hk()[i] != []:
            ...
            if max_of_all == max(max_of_all):
                ...
                listrc.append(k18)
                k21=max(max_of_all)
                max_of_all.append(k21)
                print("hydraulic height of water above water turbine,Hk (m)-------", listh1 ,"\nCrest freeboard,Rc(m)------", listrc, "\nMaximum power of the water turbine,Pkel (W/m)-----------", max_of_all)
            else:
                pass
        else:
            pass
    return max_of_all,listh1,listrc
maximum_of_scenario()
print("----------------------")
print("FINAL RESULTS")
print("hydraulic height of water above water turbine,Hk (m)-------", listh1,
"\nCrest freeboard,Rc(m)------", listrc, "\nMaximum power of the water turbine,Pkel (W/m)-----------", max(max_of_all))
